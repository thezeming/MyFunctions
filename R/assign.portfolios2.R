...
#'
#' **CAUTION:**
#' * Be careful not to do a second sort after this, since the number of observations, thus breakpoints, may change for the second sort. Finish all the sorts before this operation.
#' * This function cannot be applied to grouped tibble as the computation of breakpoints would be across all groups, instead of for each group. Use reframe() and apply it to each group respectively instead.
#'
#'
...
assign.portfolios2 <-
  function(data,
    ...
    ### this function cannot be applied to grouped tibble as the computation of breakpoints would be across all groups, instead of for each group.
    if(is.grouped_df(data)){
      stop('This function cannot be applied to grouped tibble. Use reframe and apply it to each group respectively instead.')
    }
    if (is.null(bps)) {
